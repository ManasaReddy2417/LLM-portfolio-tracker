...
WEEKS_DATA = [
    {"week_num":1,"start_date":datetime(2026,2,2),"end_date":datetime(2026,2,6),"portfolios":{
    ...
    {"week_num":2,"start_date":datetime(2026,2,9),"end_date":datetime(2026,2,13),"portfolios":{
    ...
    {"week_num":3,"start_date":datetime(2026,2,16),"end_date":datetime(2026,2,20),"portfolios":{
    ...
    {"week_num":4,"start_date":datetime(2026,2,23),"end_date":datetime(2026,2,27),"portfolios":{
        "ChatGPT": [{"stock":"NVDA","weight":30},{"stock":"TSLA","weight":25},{"stock":"META","weight":20},{"stock":"AMD","weight":15},{"stock":"COIN","weight":10}],
        "Grok":    [{"stock":"NVDA","weight":35},{"stock":"AMD","weight":25},{"stock":"MU","weight":20},{"stock":"PLTR","weight":10},{"stock":"GOOGL","weight":10}],
        "DeepSeek":[{"stock":"TQQQ","weight":50},{"stock":"SPXL","weight":30},{"stock":"TSLA","weight":20}],
        "Claude AI":[{"stock":"NVDA","weight":25},{"stock":"MSFT","weight":20},{"stock":"AVGO","weight":20},{"stock":"GOOGL","weight":15},{"stock":"META","weight":10},{"stock":"AMD","weight":10}]}},
...
def fetch_stock_data():
    ...
    for w in WEEKS_DATA:
        for p in w["portfolios"].values():
            for h in p: all_stocks.add(h["stock"])
    all_stocks.add("^GSPC")
    stock_data = {}
    for symbol in sorted(all_stocks):
        try:
            ...
            for td in all_trading_days:
                ...
                if m:
                    p = round(hist["Close"].iloc[m[0]], 2); last_known = p
                elif last_known is not None: p = last_known
                elif not hist.empty: p = round(hist["Close"].iloc[0], 2)
                else: p = 100.0
                prices.append(p)
            ...
            print(f"  {symbol}: ${prices[0]} -> ${prices[-1]} ({chg:+.2f}%)")
...
def calculate_performance():
    ...
    for wd in WEEKS_DATA:
        ...
        if not week_idx: continue
        for llm_name, portfolio in wd["portfolios"].items():
            cap = llm_results[llm_name]["current_capital"]
            stocks_detail = []
            for h in portfolio:
    ...
    for llm_name in llm_results:
        ...
        for day_idx,day in enumerate(all_trading_days):
            ...
            elif cum: cum.append(cum[-1])
            else: cum.append(INITIAL_INVESTMENT)
...
print(f"SUCCESS: docs/index.html ({len(html_content)//1024} KB)")
