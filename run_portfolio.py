...
def calculate_performance():
    ...
    for llm_name in llm_results:
        ...
        # Force the very first day to exactly $1000 for all LLMs
        if cum:
...
def generate_html(llm_results, stock_data, all_trading_days):
    ...
    # Convert UTC → Central Time (UTC-6 CST / UTC-5 CDT)
    utc_now       = datetime.utcnow()
    ct_offset     = timedelta(hours=-5) if utc_now.month in range(3,11) else timedelta(hours=-6)
    ct_now        = utc_now + ct_offset
    tz_label      = 'CDT' if utc_now.month in range(3,11) else 'CST'
    fetch_time    = ct_now.strftime('%b %d, %Y at %-I:%M %p ') + tz_label
    ranked        = sorted(llm_results.items(), key=lambda x: x[1]['total_return_pct'], reverse=True)
...
// On mobile: tighten margins and hide right-side annotations (they overlap)
